...
function canonicalJson(value) {
  ...
  if (value && typeof value === 'object') {
    // Mirror JSON.stringify's own behavior exactly: a Date (or a Mongoose
    // ObjectId, or anything else with a toJSON()) is converted via that
    // method BEFORE key-sorting, never treated as a plain object. Without
    // this, a Mongo-.lean()-sourced record's real Date instance has no own-
    // enumerable keys (Object.keys(date) === []), so canonicalJson would
    // silently collapse it to `{}` — hashing differently from the exact
    // same value after a JSON round-trip (a file read back with the same
    // field already serialized to its ISO string), which is precisely the
    // comparison computeCategoryDigest() must get right for backup-write,
    // restore-staging, and restore-apply to ever agree (research.md
    // Decision 6, third pass).
