...
/**
 * حسابات الرواتب القياسية (IAS 19) — تُعاد من الشجرة أو تُنشأ إن كانت ناقصة:
 *   2200 الرواتب المستحقة الدفع (التزام) · 2210 استقطاعات الرواتب (التزام)
 *   5110 راتب الكادر الطبي · 5120 راتب الكادر الإداري (مصروف)
 * ملاحظة: 2100 = ذمم موردين و5200 = تكلفة مواد — لا تُستخدم للرواتب أبداً.
 */
function payrollAccounts(coa) {
  const find = code => coa.find(a => String(a.code) === code);
  const ensure = (code, name, type, parent) => {
    let a = find(code);
    if (!a) {
      a = { id: code, code, name, type, parent, isGroup: false, status: 'active', balance: 0,
            normalBalance: type === 'expense' ? 'debit' : 'credit',
            description: 'أُنشئ تلقائياً لقيود الرواتب (IAS 19)', createdAt: new Date().toISOString() };
      coa.push(a);
    }
    return a;
  };
  return {
    payable:    ensure('2200', 'الرواتب المستحقة الدفع', 'liability', '2000'),
    deductions: ensure('2210', 'استقطاعات الرواتب',      'liability', '2200'),
    expMedical: ensure('5110', 'راتب الكادر الطبي',       'expense',   '5100'),
    expAdmin:   ensure('5120', 'راتب الكادر الإداري',     'expense',   '5100'),
  };
}

/**
 * حسابات PIFSS (التأمينات الاجتماعية — حصة صاحب العمل) — P0.2:
 * كانت تُرحَّل سابقاً على 5250 (غير موجود) مديناً و2200 (رواتب مستحقة — حساب
 * مختلف الغرض) دائناً، فتختفي من القوائم المالية وتختلط بالتزام رواتب آخر.
 * الآن حسابان مخصّصان تحت نفس تسلسل 5100/2000 المستخدم فعلياً في الشجرة.
 */
function pifssAccounts(coa) {
  const find = code => coa.find(a => String(a.code) === code);
  const ensure = (code, name, type, parent) => {
    let a = find(code);
    if (!a) {
      a = { id: code, code, name, type, parent, isGroup: false, status: 'active', balance: 0,
            normalBalance: type === 'expense' ? 'debit' : 'credit',
            description: 'أُنشئ تلقائياً لقيود PIFSS', createdAt: new Date().toISOString() };
      coa.push(a);
    }
    return a;
  };
  return {
    expense: ensure('5130', 'التأمينات الاجتماعية — حصة صاحب العمل (PIFSS)', 'expense', '5100'),
    payable: ensure('2600', 'التأمينات الاجتماعية المستحقة — PIFSS', 'liability', '2000'),
  };
}

/**
 * حسابات مكافأة نهاية الخدمة (EOS) — P0.2:
 * كانت تُرحَّل سابقاً على 5260 (غير موجود) مديناً و2300 "إيجار مستحق" دائناً —
 * تصادم دلالي حقيقي (التزام إيجار ≠ مخصص نهاية خدمة). الآن حسابان مخصّصان.
 */
function eosAccounts(coa) {
  const find = code => coa.find(a => String(a.code) === code);
  const ensure = (code, name, type, parent) => {
    let a = find(code);
    if (!a) {
      a = { id: code, code, name, type, parent, isGroup: false, status: 'active', balance: 0,
            normalBalance: type === 'expense' ? 'debit' : 'credit',
            description: 'أُنشئ تلقائياً لقيود مكافأة نهاية الخدمة', createdAt: new Date().toISOString() };
      coa.push(a);
    }
    return a;
  };
  return {
    expense:   ensure('5140', 'مخصص مكافأة نهاية الخدمة', 'expense', '5100'),
    liability: ensure('2700', 'مخصص مكافأة نهاية الخدمة', 'liability', '2000'),
  };
}

module.exports = { nextChildCode, suggestChildCode, repairChart, payrollAccounts, pifssAccounts, eosAccounts };
