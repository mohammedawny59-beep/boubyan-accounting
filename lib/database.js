...
// P4 — Phase B: per-tenant AppConfig isolation (research.md Decision 2),
// siblings of the three Maps above but for the single-document-per-tenant
// config, not the many-keyed entity data. `_tenantConfigDirty` is a Set (not
// a Map of key-sets like `_tenantDirty`) because a tenant's own config is
// one whole document, not many independently-dirty keys.
const _tenantConfigCaches = new Map();
const _tenantConfigDirty  = new Set();
const _tenantConfigFileTimers = new Map();
...
async function _flushTenantToMongo(tenantId) {
  ...
  const configDirty = _tenantConfigDirty.has(tenantId);
  // P4 — corrected guard: a tenant whose ONLY pending Mongo-mode change is
  // saveConfig() (no saveDB() in the same debounce window) has no entry in
  // `_tenantCaches`/`_tenantDirty` at all — the original `!db || dirty.size
  // === 0` guard would return here before the config-flush branch below
  // ever runs, silently dropping that tenant's own config write.
  if ((!db || dirty.size === 0) && !configDirty) return;
  ...
  if (configDirty) {
    _tenantConfigDirty.delete(tenantId);
    tasks.push(AppConfig.updateOne(
      { tenantId, key: 'config' },
      { $set: { tenantId, data: _tenantConfigCaches.get(tenantId), updatedAt: new Date() } },
      { upsert: true },
    ));
  }
...
// P4 — Phase D (research.md Decision 8, ninth-pass extension): the single
// authoritative entity-key list for tenant-scoped backup/restore — every
// ENTITY_KEYS member EXCEPT the two that must never be captured or replayed
// by that tooling: 'idempotencyRecords' (replaying a stale claim would
// reintroduce the duplicate-posting risk P0.12 exists to prevent) and
// 'auditLog' (a tenant restore replaces this category; excluding it is what
// keeps a restore — including of 'default', where every tool in this
// feature centralizes its own audit events — from silently reverting the
// accountability trail it is itself supposed to be recorded in).
const TENANT_BACKUP_ENTITY_KEYS = ENTITY_KEYS.filter(k => k !== 'idempotencyRecords' && k !== 'auditLog');
...
// P4 — Tenant Write Isolation (research.md Decision 1): both functions below
// previously had NO tenant scoping at all — persistUsers()'s deleteMany()
// filtered only by `id:{$nin:ids}`, and its upsert filter was only `{id}`;
// persistEntityKey()'s filter was only `{key}`. Since default's own
// buildInitialDB()/route-driven save cycle is the ONLY caller of these two
// functions (flushToMongo()'s 'users'/ENTITY_KEYS branches, persistAll()'s
// bootstrap), an ordinary default-tenant save whose user list didn't include
// some id, or whose entityChunks didn't include some key, would silently
// delete or overwrite a DIFFERENT tenant's own document sharing that same
// id/key — the two collections have no other isolation guarantee once a
// real SaaS tenant's documents exist alongside default's. Scoped to
// _defaultTenantFilter (matching legacy no-tenantId-field documents too, so
// existing default users/chunks predating multi-tenancy are updated in
// place, never duplicated) and normalized forward via `tenantId:'default'`
// in every $set, exactly mirroring the already-tenant-scoped
// _persistUsersTenant()/_persistEntityKeyTenant() pair above.
async function persistUsers(users) {
  const list = users || [];
  const ids = list.map(u => u.id).filter(Boolean);
  if (ids.length) {
    await User.deleteMany({ ..._defaultTenantFilter, id: { $nin: ids } });
  } else {
    await User.deleteMany(_defaultTenantFilter);
  }
  ...
  if (configDirty && _configCache) {
    tasks.push(
      AppConfig.updateOne(
        ...
        { $set: { tenantId: 'default', data: _configCache, updatedAt: new Date() } },
        { upsert: true },
      ),
    );
  }
...
async function initConfig(configFile, defaultConfig) {
  ...
  // P4 — restores a fix an earlier Phase-B rewrite incorrectly dropped: every
  // one of this function's own AppConfig call sites is scoped to
  // _defaultTenantFilter + normalize-forward, exactly like
  // persistUsers()/persistEntityKey(). Without this, once any non-default
  // tenant has its own AppConfig document (permitted by the compound unique
  // index), these unscoped-by-tenant `{key:'config'}` reads/writes could
  // read or overwrite the wrong tenant's document.
  const cfgDoc = await AppConfig.findOne({ ..._defaultTenantFilter, key: 'config' }).lean();
  if (cfgDoc?.data) {
...
// P4 — مسار ملف إعدادات المستأجر في وضع الملفات (sibling of _tenantFilePath,
// data/tenants/<tid>.config.json — same sanitization, same directory).
function _tenantConfigFilePath(tid) {
  if (!_dataFile) return null;
  const safe = String(tid).replace(/[^a-zA-Z0-9_-]/g, '_');
  return require('path').join(require('path').dirname(_dataFile), 'tenants', safe + '.config.json');
}

// P4 — Phase D: lets a standalone CLI tool (scripts/tenant-backup.js,
// scripts/tenant-restore.js) use the SAME _tenantFilePath()/
// _tenantConfigFilePath() path logic the live app uses, without running the
// full initDB() (Mongo connect, migration, default-tenant bootstrap) that
// module normally requires to populate _dataFile — those side effects are
// wrong for a short-lived, single-tenant tool. Sets ONLY the module-private
// _dataFile this pair of path helpers reads; nothing else.
function _setDataFileForTooling(dataFile) {
  _dataFile = dataFile;
}
...
// P4 — Phase B: debounced per-tenant config file write (sibling of
// _scheduleTenantFilePersist above, but for _tenantConfigCaches/
// _tenantConfigFileTimers instead of the entity-data pair).
const _tenantConfigFilePersistFailures = new Map();
function _scheduleTenantConfigFilePersist(tid) {
  const fp = _tenantConfigFilePath(tid);
  if (!fp) return;
  if (_tenantConfigFileTimers.has(tid)) clearTimeout(_tenantConfigFileTimers.get(tid));
  _tenantConfigFileTimers.set(tid, setTimeout(() => {
    _tenantConfigFileTimers.delete(tid);
    try {
      fs.ensureDirSync(require('path').dirname(fp));
      _atomicWriteJsonSync(fp, _tenantConfigCaches.get(tid) || {});
      _tenantConfigFilePersistFailures.delete(tid);
    } catch (e) {
      console.error('❌ Tenant config file persist failed (' + tid + '):', e.message);
      const failures = (_tenantConfigFilePersistFailures.get(tid) || 0) + 1;
      _tenantConfigFilePersistFailures.set(tid, failures);
      const delay = Math.min(30000, 1000 * Math.pow(2, Math.min(failures, 5)));
      console.error(`   ↳ سيُعاد المحاولة خلال ${delay / 1000}ث (إعدادات مستأجر ${tid}, محاولة ${failures})`);
      setTimeout(() => _scheduleTenantConfigFilePersist(tid), delay);
    }
  }, PERSIST_DEBOUNCE_MS));
}
...
// P4 — Phase B: warm a non-default tenant's config cache (research.md
// Decision 2). Unlike warmTenantCache() above, this does NOT skip file
// mode — loadConfig()'s own non-default branch (below) has no lazy,
// loadDB()-style inline fallback; it strictly requires the cache to already
// be warmed and throws otherwise (spec.md FR-022's fail-closed requirement),
// so both Mongo mode and file mode must actually populate the cache here.
async function warmTenantConfigCache(tenantId) {
  if (tenantId === 'default' || _tenantConfigCaches.has(tenantId)) return;
  let cfg = null;
  if (_useFileFallback) {
    const fp = _tenantConfigFilePath(tenantId);
    if (fp && fs.existsSync(fp)) {
      try { cfg = fs.readJsonSync(fp); } catch { cfg = null; }
    }
  } else {
    const doc = await AppConfig.findOne({ tenantId, key: 'config' }).lean();
    if (doc?.data) cfg = doc.data;
  }
  _tenantConfigCaches.set(tenantId, cfg ? deepMerge(_defaultConfig || {}, cfg) : { ...(_defaultConfig || {}) });
}

// P4 — Phase B: per-tenant branch (research.md Decision 2). `default`'s own
// behavior below is completely UNCHANGED — same signature, same synchronous
// return, every existing call site works exactly as before (spec.md
// FR-022). A non-default tenant strictly requires warmTenantConfigCache()
// to have already run for it (wired into tenantMiddleware.js) — this never
// silently falls back to _defaultConfig/another tenant's config on a cache
// miss, matching loadDB()'s own "fail closed, never leak default's cache"
// discipline, but via a hard throw rather than an auto-created empty shape,
// since config (unlike entity data) has no natural "start empty" state a
// caller could safely proceed with.
function loadConfig() {
  const tid = _currentTenantId();
  if (tid !== 'default') {
    if (_tenantConfigCaches.has(tid)) return _tenantConfigCaches.get(tid);
    throw new Error(`Config not warmed for tenant "${tid}" — warmTenantConfigCache() must run before loadConfig()`);
  }
...
function saveConfig(cfg) {
  ...
  if (tid !== 'default') {
    ...
    if (_useFileFallback) {
      _scheduleTenantConfigFilePersist(tid);
    } else {
...
async function _flushAllTenantsOnShutdown() {
  ...
  _tenantTimers.clear();
  // P4 — Phase B: cancel pending config-file timers too, same rationale.
  for (const timer of _tenantConfigFileTimers.values()) clearTimeout(timer);
  _tenantConfigFileTimers.clear();
  ...
  if (_useFileFallback) {
    ...
    // P4 — Phase B: unconditionally flush every cached tenant config too,
    // mirroring the entity-data loop immediately above exactly (never
    // "pending timers only" — a tenant mid-retry after a transient write
    // failure has no entry in _tenantConfigFileTimers at all, same gap this
    // codebase already avoids for entity data).
    for (const tid of _tenantConfigCaches.keys()) {
      const fp = _tenantConfigFilePath(tid);
      if (!fp) continue;
      try {
        fs.ensureDirSync(require('path').dirname(fp));
        ...
        console.error(`❌ Tenant config shutdown flush failed (${tid}):`, e.message);
      }
    }
    return;
  }
  ...
  // redundant upserts at worst, never a lost write. P4: also includes
  // every tenant with a dirty or cached config, so a tenant whose only
  // pending change is saveConfig() is never skipped.
  const tenantIds = new Set([
    ..._tenantDirty.keys(), ..._tenantCaches.keys(),
    ..._tenantConfigDirty, ..._tenantConfigCaches.keys(),
  ]);
...
async function initDB({ mongoUri, dataFile, configFile, defaultConfig, buildInitialDB, migrateDB }) {
  ...
  // P4 — Phase B: declared together with _tenantConfigCaches/_tenantConfigDirty/
  // _tenantConfigFileTimers themselves (T008), specifically so this
  // registration cannot be dropped the way it was for the entity-data Maps
  // during an earlier milestone's own first Phase-B rewrite. Timer handles
  // are cleared before their Map is cleared, exactly like the two sibling
  // timer maps above — each closure captures the OLD backend's state and
  // must not fire against it after a same-process backend switch.
  for (const t of _tenantConfigFileTimers.values()) clearTimeout(t);
  _tenantCaches.clear();
  _tenantDirty.clear();
  _tenantTimers.clear();
  _tenantFileTimers.clear();
  _tenantFilePersistFailures.clear();
  _tenantConfigCaches.clear();
  _tenantConfigDirty.clear();
  _tenantConfigFileTimers.clear();
...
module.exports = {
  ...
  runAsTenant,
  ENTITY_KEYS,
  // P4 — Phase D: exported for scripts/tenant-backup.js / scripts/tenant-
  // restore.js (tasks.md T027, T028) — see each function's own comment.
  TENANT_BACKUP_ENTITY_KEYS,
  _tenantFilePath,
  _tenantConfigFilePath,
  _setDataFileForTooling,
  isFileFallbackMode,
