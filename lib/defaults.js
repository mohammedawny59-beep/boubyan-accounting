...
const DEFAULT_COA = [
  ...
  { id:'1900', code:'1900', name:'حساب معلّق — تسوية مؤقتة (Suspense)', type:'asset', parent:'1000', balance:0 },
  // ── الأصول الثابتة ────────────────────────────────
  ...
  { id:'2600', code:'2600', name:'التأمينات الاجتماعية المستحقة — PIFSS', type:'liability', parent:'2000', balance:0 },
  { id:'2700', code:'2700', name:'مخصص مكافأة نهاية الخدمة', type:'liability', parent:'2000', balance:0 },
  // ── حقوق الملكية ─────────────────────────────────
  ...
  { id:'5130', code:'5130', name:'التأمينات الاجتماعية — حصة صاحب العمل (PIFSS)', type:'expense', parent:'5100', balance:0 },
  { id:'5140', code:'5140', name:'مخصص مكافأة نهاية الخدمة', type:'expense', parent:'5100', balance:0 },
  ...
  { id:'5230', code:'5230', name:'مصروف نقص/تسوية مخزون', type:'expense', parent:'5200', balance:0 },
...
const DEFAULT_ROLES = {
  ...
  accountant: {
    name: 'محاسب',
    color: '#2dd4bf',
    // R2 — 'vendorbills' (AP UI) rides on the existing 'vendors' grant;
    // 'patients'/'patientreceivables' (AR UI) ride on the existing 'manual'
    // grant below — without these tab ids, applyPermissions() hides every
    // R2 nav button for this role even though the backend already
    // authorizes the underlying actions.
    tabs: ['dashboard','manual','expenses','vendors','vendorbills','patients','patientreceivables','vouchers','cashrecon','insurance','payroll','journal','coa','ledger','recurring','financials','reports','ai'],
    actions: {
      // P0.8 — 'edit'/'delete' added to 'manual' so the accountant role (the
      // natural owner of patient-AR collections/credits/write-offs, per
      // existing P0.7 precedent where 'edit'=pay/credit, 'delete'=cancel on
      // an equivalent subledger domain) can operate the new AR routes —
      // receptionist keeps 'view'/'add' only (can record a receivable, same
      // as they already could for daily income, but not reverse one).
      dashboard: ['view','export'], manual: ['view','add','edit','delete'], expenses: ['view','add','edit'],
  ...
  // P3 — public demo visitor: broad READ access across the record-workspace
  // and financial-statement surface (so a visitor can explore dashboard,
  // journal, patients/doctors/vendors/assets detail, and all 5 reports) but
  // zero write access anywhere, and zero access to upload/telegram/settings/
  // ai/inventory. No production user is ever assigned this role — it only
  // matters on a DEMO_MODE instance's own seeded roles. Fixed-asset routes
  // are additionally gated by requireAdminOrDemoView (server.js) since they
  // sit behind requireAdminAction, not the tab/action policy this object
  // otherwise fully controls.
  demo: {
    name: 'زائر تجريبي',
    color: '#f97316',
    tabs: ['dashboard','manual','doctors','expenses','vendors','vendorbills','patients','patientreceivables','payroll','insurance','journal','coa','ledger','financials','reports'],
    actions: {
      dashboard: ['view'], manual: ['view'], doctors: ['view'], expenses: ['view'],
      vendors: ['view'], vendorbills: ['view'], patients: ['view'], patientreceivables: ['view'],
      payroll: ['view'], insurance: ['view'], journal: ['view'], coa: ['view'],
      ledger: ['view','export'], financials: ['view','export'], reports: ['view','export'],
    }
  }
};
