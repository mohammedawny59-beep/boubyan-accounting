...
## Audit trail (NEW, eighth pass, HIGH — research.md Decision 23, spec.md FR-037, per this project's own constitution Principle XI)

On any terminal outcome — a written backup file, or a hard-fail from the default-duplicate pre-flight or the non-default existence/active-status pre-flight — call the canonical `appendAuditEvent()` (`lib/auditLog.js`) exactly once, never a hand-rolled event object, written to `default`'s own `auditLog` (mirroring `lib/tenantMiddleware.js`'s own existing precedent for cross-tenant accountability logging, not the target tenant's own log). `action:'tenant.backup'`, `resourceType:'tenant'`, `resourceId:<--tenant= value>`, `outcome:'success'|'failure'`, `metadata:{recordCounts, categoryDigests}` on success. Mongo-mode mechanics: same direct `EntityChunk{tenantId:'default',key:'auditLog'}` read-modify-write the restore contract's own Step 6 uses — see that contract for the exact sequence. No audit event is written for a plain early exit with no attempted action (e.g. missing `--tenant=`).
...
11. **Audit event (NEW, eighth pass, MANDATORY)**: a successful backup appends exactly one event to `default`'s own `auditLog` with `action:'tenant.backup'`, `resourceId` equal to the target tenantId, `outcome:'success'` — confirm it is absent from the *target* tenant's own `auditLog` if the target is non-`default`. A hard-failed backup (e.g. the existence/active-status pre-flight) appends exactly one event with `outcome:'failure'` to the same log instead.
