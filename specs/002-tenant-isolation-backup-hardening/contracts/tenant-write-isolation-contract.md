...
**Revised through Design Remediation Passes 1-4** — Section A is unchanged (survived all four `/speckit-analyze` rounds). Section B is substantially redesigned (research.md Decision 2 *(fourth-pass correction: previously mis-cited as "Decision 2, 3" — Decision 3 is the unrelated Telegram/scheduler boundary; Section B's only source decision is Decision 2)*); Pass 2 additionally restores a dropped default-tenant `AppConfig` scoping fix, corrects `_flushTenantToMongo()`'s early-return guard, and adds file-mode config-persist timer/shutdown wiring; Pass 4 corrects the shutdown-wiring design itself (below) to match the real sibling code's unconditional-flush behavior.
...
**File-mode config persistence — timer map and shutdown wiring (NEW second pass, corrected fourth pass)**: `_scheduleTenantConfigFilePersist(tid)` registers into a new `_tenantConfigFileTimers = new Map()` (mirroring the existing `_tenantFileTimers`/`_scheduleTenantFilePersist` pair, `lib/database.js:691-712`), added to the P0.11 reset block (with the `clearTimeout`-first treatment its live-handle nature requires, third pass). `_flushAllTenantsOnShutdown()` (`lib/database.js:823-853`) is extended to **unconditionally** write every entry in `_tenantConfigCaches.keys()` via `_atomicWriteJsonSync(_tenantConfigFilePath(tid), _tenantConfigCaches.get(tid))` before shutdown completes — exactly paralleling the real, current `:831-841`'s unconditional `_tenantCaches`-based flush, **not** by iterating `_tenantConfigFileTimers` for "pending" entries only (the second pass's original, incorrect instruction — a tenant mid-retry after a transient write failure has no tracked timer entry at all, mirroring the identical gap in the existing entity-data retry path at `:703-709`, so a pending-only flush would miss exactly the tenant most at risk). Omitting durable shutdown flush entirely would reintroduce the SIGTERM data-loss bug class this codebase's own P0.5 milestone already closed for entity data.
