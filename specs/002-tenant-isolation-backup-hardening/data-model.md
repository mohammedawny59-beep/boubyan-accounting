...
**Revised in Design Remediation Pass 1, corrected in a second pass, and corrected again in a third pass.** The second `/speckit-analyze` re-verification found 5 CRITICAL, 6 HIGH, 2 MEDIUM new/incompletely-closed defects in Pass 1 itself (a dropped default-tenant `AppConfig` scoping fix, an unscoped `entityChunks` delete threatening the lock/idempotency store, a `runId`-keyed checkpoint with no tenant lookup path, an internally-contradictory stale-lock policy, an underspecified digest canonicalization, among others — marked "second pass" inline). A third `/speckit-analyze` re-verification of Pass 2 then found 3 CRITICAL, 2 HIGH, 4 MEDIUM further defects (a missing `clearTimeout` step in the new config-file-timer reset logic, a digest computed over structurally inconsistent `_id`-bearing vs. `_id`-stripped representations at different points in the pipeline, and several stale decision-number cross-references — marked "third pass" inline). No new Mongoose *models* are added even after all three passes — the restore lock (below) deliberately reuses the existing `EntityChunk` schema/index rather than introducing one. Two logical file formats are substantially redesigned; new in-memory state is added (now including a config-file timer map), all participating in the existing cross-backend reset.
