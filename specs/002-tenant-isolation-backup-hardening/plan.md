...
**Branch**: `002-tenant-isolation-backup-hardening` | **Date**: 2026-09-08 (Design Remediation Pass 13) | **Spec**: [spec.md](./spec.md)

**Input**: Feature specification from `specs/002-tenant-isolation-backup-hardening/spec.md`. Revised through thirteen rounds of adversarial `/speckit-analyze` + remediation: Pass 1 closed 17 findings (8 CRITICAL/6 HIGH/3 MEDIUM) from the first analyze; Pass 2 closed 13 findings (5 CRITICAL/6 HIGH/2 MEDIUM) a re-verification found in Pass 1 itself; Pass 3 closed 9 findings (3 CRITICAL/2 HIGH/4 MEDIUM) a re-verification found in Pass 2 itself; Pass 4 closed 11 findings (1 CRITICAL/6 HIGH/4 MEDIUM) a re-verification found in Pass 3 itself; Pass 5 closed 9 findings (1 CRITICAL/5 HIGH/3 MEDIUM) a re-verification found in Pass 4 itself; Pass 6 closed 8 findings (0 CRITICAL/1 HIGH/7 MEDIUM) a re-verification found in Pass 5 itself; Pass 7 closed 9 findings (1 CRITICAL/2 HIGH/6 MEDIUM) a re-verification found in Pass 6 itself; Pass 8 closed 11 unique findings (0 CRITICAL/7 HIGH/4 MEDIUM — 14 raw across independently-overlapping review dimensions before deduplication) a re-verification found in Pass 7 itself; Pass 9 closed 19 findings (2 CRITICAL/9 HIGH/8 MEDIUM) a re-verification found in Pass 8 itself; Pass 10 closed 15 findings (0 CRITICAL/7 HIGH/8 MEDIUM) a re-verification found in Pass 9 itself; Pass 11 closed 19 findings (0 CRITICAL/3 HIGH/14 MEDIUM/2 LOW) a re-verification found in Pass 10 itself; Pass 12 closed 15 findings (0 CRITICAL/10 HIGH/4 MEDIUM/1 LOW) a re-verification found in Pass 11 itself; **Pass 13 (this revision)** closes 17 findings (0 CRITICAL/7 HIGH/8 MEDIUM/2 LOW) a re-verification found in Pass 12 itself.
...
**Pass 11's re-verification of Pass 10 found no new CRITICAL. The most substantive finding is a scope-widening rather than a bug: research.md Decision 23's own "exactly one event is still guaranteed" audit claim (ninth pass) had gone stale the moment Decision 25's Step 4a confirmation gate was introduced in that same pass — an operator's ordinary decline at Step 4a is a fourth genuine terminal outcome of a privileged, lock-holding invocation the "Step 1, Step 2, or Step 5/6" enumeration never covered, silently leaving a deliberate cancellation unaudited.** Closed by widening the audit-event requirement to a fourth terminal-outcome family with its own `outcome:'cancelled'` value, propagated to the restore contract, T053a/T056d, data-model.md, and spec.md's FR-037/FR-039 — the sole remaining exception is a raw SIGINT/SIGTERM kill during the prompt, for which no event can fire before the process dies. The remaining findings closed a cluster of citation and self-consistency defects, most of them fresh instances of failure classes this document set has repeatedly named: an off-by-one `scripts/restore.js:20-23`→`:21-24` citation for the `ask()` helper, duplicated verbatim into research.md and this contract set's own restore contract (a copy-paste of one wrong citation into the other); research.md's own header citing the new retention-collision test as "(12/14a)" instead of "(12a/14a)"; a stray "(this revision)" tag left on the superseded Pass-9 sentence in research.md, data-model.md, and the restore contract, while this document had already correctly moved it — an inconsistent propagation of the identical fix across four documents; research.md Decision 24's own tenth-pass rewrite silently dropping the chronological-sort clause its predecessor paragraph had (restored); this document's own Constitution Check table overclaiming Pass 10 was "the first zero-CRITICAL round since Pass 6" when Pass 8 was also zero (corrected above); two places in this document mislabeling the Step-4a-blocks-pre-existing-tests finding as CRITICAL, contradicting this same document's own "0 CRITICAL" Pass 10 tally and `tasks.md`'s own matching HIGH-labeled bullet for the identical finding (both corrected to HIGH above); `tasks.md`'s own Task Quality Gate bullet claiming a Pass-9 origin it never actually had; `tasks.md`'s own blanket `--yes`/`RESTORE_YES=1` requirement bullet claiming "every test in this Phase" while naming tests from two other phases; this document's own Documentation manifest never actually updated for data-model.md's or quickstart.md's real Pass-10 edits, despite Pass 10's own commit message claiming a blanket "gain Pass 10 entries" (corrected above, along with backfilling the identical gap for those two files' own Pass-9 content, which had the same staleness one pass earlier); and spec.md's own Pass-9 note transposing its HIGH/MEDIUM split, the identical severity-count-disagreement failure class Pass 10 itself hunted for and fixed one paragraph above in that same file, in the very next paragraph Pass 10 did not check. This pass fixes all of the above at the design-document level; none of it has been implemented as source code, and Phase A remains **unchanged** — it has now survived eleven rounds of adversarial analysis intact.

**Pass 12's re-verification of Pass 11 found no new CRITICAL — the third consecutive zero-CRITICAL round — but the largest HIGH count since Pass 9's own two-CRITICAL round, almost every one of them a fresh instance of the same recurring meta-pattern: Pass 11's own new content, written specifically to close a completeness gap and a citation gap, introduced fresh instances of both classes of defect inside itself.** Most consequentially: research.md Decision 23's own new "sole exception is a raw signal kill" audit-guarantee sentence (Pass 11's own Step 4a widening) both misattributed its residual-risk citation — claimed as "this same decision's own tenth-pass residual-risk addendum," when that addendum lives entirely under Decision 25, a separate section — and overstated the exception's own scope, since a non-signal hard crash (e.g. an unhandled exception) during the identical window has the identical effect, not covered by "signal kill" specifically. Both corrected, and the correction propagated everywhere the same narrow framing had been copied: the restore contract, data-model.md, tasks.md, and spec.md. This document's own Constitution Check table (row XIII) carried the largest single defect of the round: a freshly-fabricated "CRITICAL-count history" digit sequence, introduced in the very sentence meant to correct a *different* overclaim, that zeroed out Passes 1-5's real CRITICAL counts (8, 5, 3, 1, 1) against this same document's own accurate Input-line tallies three sections above — corrected. This document's own Documentation manifest reproduced the identical "manifest claims completeness but isn't" gap it was praised, in Pass 11's own narrative, for having just closed: the `data-model.md`/`quickstart.md` entries got real Pass-11 content, but `spec.md`, `quickstart.md` (a second, separate omission — Pass 11's own edit to this specific file was never entered even after the first backfill), `tasks.md`, and `contracts/tenant-backup-contract.md` did not, despite all four being genuinely edited in the Pass-11 commit — all four backfilled here. The backup contract's own header had an analogous self-referential gap: the eleventh-pass sentence fixing its "never mentions Pass 10" omission was itself an eleventh-pass edit the header's own opening pass-list never named — fixed, and the investigation surfaced a genuine, previously-unnoticed structural mismatch: `tasks.md`'s T026/T036 numbered the backup test suite 1-through-14-plus-14a (15 items, `auditLog` exclusion at position 13) while the backup contract's own canonical Test contract numbered the identical suite 1-through-12-plus-8a/12a (14 items, `auditLog` exclusion at 8a) — genuinely different ordinals and a different total count for the same tests in two documents both meant to be authoritative. `tasks.md` renumbered to match the contract exactly; no test content changed. One open-ended-sweep finding stands apart from the citation/consistency cluster, a real design consideration rather than a documentation defect: Pass 11's own new `outcome:'cancelled'` audit value is not yet distinguishable from `'success'` in `public/index.html`'s existing, unmodified Audit Trail viewer (a hardcoded binary badge, a two-option outcome filter) — acknowledged as a fourth known residual risk in research.md Decision 23, deliberately not closed in this phase, since that UI lives entirely outside this feature's own scope and this phase touches no source code of any kind. This pass fixes all of the above at the design-document level; none of it has been implemented as source code, and Phase A remains **unchanged** — it has now survived twelve rounds of adversarial analysis intact.

**Pass 13's re-verification of Pass 12 found no new CRITICAL — the fourth consecutive zero-CRITICAL round.** Most substantive: research.md Decision 23's own "Twelfth-pass broadening" sentence — written specifically to fix Pass 11's misattributed citation — silently redefined the residual-risk window in the same breath: the sentence immediately before it correctly describes the exception as covering the process dying *while the prompt still awaits input*, but the "broadening" sentence redescribed the window as starting only "after Step 4a's decline is recognized," a materially later, narrower window that would have silently excluded the primary scenario (a signal delivered before the operator has typed anything) — the very scenario every other document this fix was propagated to (the restore contract, data-model.md, tasks.md, spec.md) consistently describes. Corrected to state the same window consistently everywhere. This document's own Documentation manifest reproduced the "manifest claims completeness but isn't" gap for a third consecutive round: Pass 12's own commit backfilled four files' entries but never added one for research.md or data-model.md — the two files Pass 12 itself edited most substantively — both backfilled here, along with expanding the spec.md and tasks.md entries, which had credited only one of several real Pass-12 edits each (spec.md's entry named only FR-039's broadening, omitting FR-037's identical and more central edit, the new Pass-12 revision note, and the Status-line advance; tasks.md's entry named only T026/T036, omitting the parallel T031a/T034a/T035a/two-Task-Quality-Gate-bullet cross-reference updates the same renumbering required). spec.md's FR-037 itself had an unclosed parenthesis, a direct consequence of Pass 12's own text insertion consuming what had been the outer group's closing paren without supplying a replacement — fixed. The backup contract's own header reproduced, one level deeper, the exact self-referential omission its own immediately-preceding sentence had just finished describing and fixing for the eleventh pass: a new "Twelfth pass" sentence was added without extending the header's own opening pass-list to include "twelfth" — fixed. Two smaller citation-staleness fixes, both surfaced by the T026/T036 renumbering: research.md's own header still cited the collision test's tasks.md-side number as "14a" after Pass 12 had already renumbered it to "12a"; and a Pass-8-era historical line in tasks.md had one T026 case-number cross-reference the Pass-12 renumbering swept past. One MEDIUM finding closed a convention gap: quickstart.md's own header — which this document set's own established convention requires to explicitly note even a no-content-change pass — was never updated to acknowledge Pass 12; closed with an explicit no-change note, matching the Pass-3 precedent. This pass fixes all of the above at the design-document level; none of it has been implemented as source code, and Phase A remains **unchanged** — it has now survived thirteen rounds of adversarial analysis intact.
...
| XI. Auditability | **PASS, first analyzed Pass 8, that analysis found incomplete and completed Pass 9** | This row certified "Unchanged" in every one of the first seven passes without ever re-deriving that status against this feature's own new capability — an assertion, not an analysis. Pass 8, HIGH: the constitution's own Principle XI requires new privileged functionality to use `appendAuditEvent()` unless a documented spec exception exists; `scripts/tenant-restore.js` (a full-tenant-replace operation) and `scripts/tenant-backup.js` had neither the audit-logging requirement nor a documented exception, across all seven prior passes. Closed via new Decision 23/FR-037: both tools now call `appendAuditEvent()` exactly once per terminal outcome, writing to `default`'s own audit log (never the target tenant's, since Step 5's own apply would otherwise be capable of destroying the record of itself). **Pass 9, CRITICAL — Pass 8's own fix was itself incomplete**: `auditLog` was never excluded from `TENANT_BACKUP_ENTITY_KEYS`, so a restore *of* `default` (a normal, supported target) would delete-and-reinsert `default`'s own `entityChunks` category — `auditLog` included — silently reverting the centralized log every tenant's events are concentrated into, system-wide. Closed by extending research.md Decision 8's exclusion filter. **Pass 9, also**: the "mirrors `lib/tenantMiddleware.js`'s own existing precedent" claim was corrected — only the *purpose* is shared; the actual write mechanism (a direct `EntityChunk` read-modify-write) is new to this codebase. A related residual risk is now explicitly acknowledged rather than silently assumed away: `default`'s own live server, if running, can still overwrite the audit event via its own stale-cache mechanism (Decision 19's own mechanism, unaddressed by the `TENANT_BACKUP_ENTITY_KEYS` fix, which only protects against restore, not the live server's ordinary writes) — documented, not architecturally eliminated, for the same "disproportionate to this feature's scope" reasoning Decision 19 itself already established. Retrofitting the existing, unmodified whole-instance `scripts/backup.js`/`scripts/restore.js` remains explicitly out of scope, consistent with this feature's standing "never touch those two files" boundary — **not**, as this row previously claimed, because the constitution itself records their absence of audit logging as "pre-existing technical debt" (it does not; that citation was corrected in Pass 9 — the constitution's own recorded technical debt is a different, unrelated set of direct `db.auditLog` call sites). **Pass 11, HIGH — the Pass-9 fix's own "exactly one event is still guaranteed" scope statement had gone stale**: Decision 25's Step 4a confirmation gate was introduced in the same Pass 9 that wrote that scope statement, but the statement itself was never revisited against it — an operator's ordinary decline at Step 4a (lock released, checkpoint left staged, exit 0) is a fourth genuine terminal outcome of a privileged invocation that "Step 1, Step 2, or Step 5/6" never covered, so no audit event fired for a deliberate cancellation at all, contradicting Principle XI's own unconditional text. Closed by widening the requirement to four terminal-outcome families, with a new `outcome:'cancelled'` value for the Step 4a case. The one remaining gap, unchanged and still not architecturally solved: a hard process termination (signal-delivered or otherwise — **Pass 12**: not signal-specific) during the Step 4a prompt itself still appends no event, since the process dies before any write can occur — the sole acknowledged exception to "exactly one event guaranteed," documented in research.md Decision 25's own tenth-pass residual-risk addendum. **Pass 12, additionally**: an open-ended sweep found this pass's own new `outcome:'cancelled'` value is not yet distinguishable from `'success'` in `public/index.html`'s existing, unmodified Audit Trail viewer — acknowledged as a fourth known residual risk in research.md Decision 23, out of scope to close since that UI is existing, unmodified source code entirely outside this feature's boundary. |
| XII. Deployment Control | N/A this phase | Unchanged. |
| XIII. Spec Kit Usage | **PASS** | `/speckit-analyze` → Pass 1 → `/speckit-analyze` → Pass 2 → `/speckit-analyze` → Pass 3 → `/speckit-analyze` → Pass 4 → `/speckit-analyze` → Pass 5 → `/speckit-analyze` → Pass 6 → `/speckit-analyze` → Pass 7 → `/speckit-analyze` → Pass 8 → `/speckit-analyze` → Pass 9 → `/speckit-analyze` → Pass 10 → `/speckit-analyze` → Pass 11 → `/speckit-analyze` → Pass 12 → `/speckit-analyze` → Remediation Pass 13 (this document's current state) → a fourteenth `/speckit-analyze` re-verification next, per the user's own gate — the process continues to work exactly as intended: each re-verification round, including of Pass 3's own citation "fixes," Pass 4's own incomplete cache-staleness fix, an FR present since the original spec that six passes of mechanics-focused review never checked was actually implemented (Pass 7), an entire constitution principle (Auditability) no pass had ever checked this feature's own new capability against (Pass 8), Pass 8's own fix to that same principle found itself incomplete plus a *second* constitutional principle (IV) Pass 8's own "swept every principle" claim had itself missed (Pass 9), a real design bug in Pass 9's own new retention mechanism plus a cluster of "Pass 9 claimed X was fixed everywhere, but wasn't" findings recurring inside the very pass that added a dedicated review dimension to guard against that exact failure class (Pass 10), the same row XI fix's own scope statement going stale the moment Pass 9's own Step 4a gate was introduced in that same pass, plus another cluster of citation/self-consistency defects including a fresh instance of the "claimed fixed everywhere" pattern turning up in this very document's own Documentation manifest (Pass 11), Pass 11's own new content reproducing the identical citation-error and completeness-gap classes it was written to fix, including a fabricated CRITICAL-count digit sequence in this very row (Pass 12), and — this round — Pass 12's own new content reproducing the identical pattern one level deeper: a citation fix (the SIGINT/SIGTERM misattribution) that itself silently redefined the residual-risk window it was correcting, and a documentation-manifest backfill that itself omitted the two files it edited most substantively, has caught what the round(s) before it missed, with total findings (17 → 13 → 9 → 11 → 9 → 8 → 9 → 14 raw/11 unique → 19 → 15 → 19 → 15 → 17) not monotonically shrinking round-over-round but each round finding genuinely new, previously-unexamined material or genuinely incomplete prior fixes rather than exhausting a fixed, shrinking list — Passes 10 through 13 all closed zero CRITICAL findings, the first four-round zero-CRITICAL run of the entire process (**eleventh-pass correction, still accurate**: this row previously claimed Pass 10 was "the first round with zero CRITICAL findings since Pass 6," directly contradicted by this same table's own numbers — Pass 8 also closed zero; Pass 10 was, at the time, only the second such round, and the run has since grown to four), the CRITICAL-count history being 8 → 5 → 3 → 1 → 1 → 0 → 1 → 0 → 2 → 0 → 0 → 0 → 0 for Passes 1-13 (**twelfth-pass correction**: this line previously mis-rendered the sequence as "0 → 0 → 0 → 0 → 0 → 0 → 1 → 0 → 2 → 0 → 0," falsely zeroing out Passes 1-5 — which in fact closed 8, 5, 3, 1, and 1 CRITICAL findings respectively, per this document's own Input line above — while Passes 6-11 happened to be rendered correctly; the fabricated sequence directly contradicted this same document's own accurate tallies a few dozen lines earlier, an error introduced by the very sentence written to fix a different, adjacent overclaim; **thirteenth-pass verification**: the corrected sequence was independently re-derived digit-by-digit against this document's own Input line one round later and confirmed accurate, including the new Pass-13 digit appended here), a signal that architectural soundness is not regressing even as documentation-consistency and self-verification-completeness findings continue, without overstating it as a longer convergence trend than the data actually shows. |
...
│                            NEW, Pass 11 (twelfth-pass correction — this entry was never actually
│                            updated for Pass 11's own real edits to this file, reproducing the exact
│                            manifest-completeness gap Pass 11 itself claimed to have closed for
│                            data-model.md/quickstart.md): Status line advanced from a stale "Pass 9
│                            complete" (never updated through all of Pass 10) to "Pass 11 complete";
│                            two new "Design Remediation Pass N note" paragraphs added (Pass 10's own,
│                            backfilled retroactively since it had never been written, and Pass 11's
│                            own); the Pass-9 note's own transposed HIGH/MEDIUM split corrected; FR-037
│                            widened to a fourth audited terminal outcome (Step 4a's own decline);
│                            FR-038's guarantee text corrected from a bookkeeping "count" framing to
│                            the real cross-tenant-deletion guarantee the Pass-10 fix actually provides;
│                            FR-039 gains the Pass-10 residual-risk acknowledgment and the Pass-11
│                            audit-widening cross-reference. NEW, Pass 12 (thirteenth-pass
│                            correction — this entry named only one of four real Pass-12 edits to this
│                            file): Status line advanced to "Pass 12 complete"; a new "Design
│                            Remediation Pass 12 note" paragraph added; **both** FR-037 and FR-039
│                            (not FR-039 alone) gain the identical SIGINT/SIGTERM-only exception
│                            language broadened to cover any hard process termination, not signals
│                            specifically (matching research.md Decision 23's own twelfth-pass
│                            broadening — FR-037 is the more central of the two, since it is where the
│                            "exactly one event... sole exception" language actually originates;
│                            FR-039 only cross-references it); FR-039 additionally gains a brief
│                            cross-reference to the new Audit-Trail-UI residual risk documented in
│                            research.md Decision 23. NEW, Pass 13: FR-037's own Pass-12 insertion had
│                            left an outer parenthetical unclosed, garbling the sentence's nesting —
│                            fixed.
├── plan.md               # This file.
...
│                            prompt; the quiesce+lock+confirmation-wait composition window). NEW,
│                            Pass 11: Decision 23 widened to a fourth audited terminal outcome (Step
│                            4a's own decline, `outcome:'cancelled'`); Decision 25's own signal-
│                            handling addendum gains a corrected `scripts/restore.js` citation
│                            (`:21-24`, was `:20-23`); Decision 24's rewritten "Decision" paragraph
│                            has its dropped chronological-sort clause restored; header's "(12/14a)"
│                            corrected to "(12a/14a)" and its stray "(this revision)" tag moved off
│                            the superseded Pass-9 sentence. NEW, Pass 12 (thirteenth-pass correction
│                            — this entry was never actually updated for Pass 12's own real edits to
│                            this file, the single most substantively edited file of that pass,
│                            reproducing the exact manifest-completeness gap Pass 12 itself claimed to
│                            have closed for four other files in the same commit): Decision 23's own
│                            audit-guarantee sentence corrected — its SIGINT/SIGTERM citation, wrongly
│                            attributed to itself, now correctly points to Decision 25; its exception
│                            language broadened from "signal kill" to "any hard process termination"
│                            (window definition itself corrected Pass 13, see below); a new "fourth
│                            known residual risk" paragraph added, documenting that the new
│                            `outcome:'cancelled'` audit value is not yet distinguishable from
│                            `'success'` in `public/index.html`'s existing Audit Trail UI; header
│                            extended to "twelve" rounds with a full Pass-12 narrative paragraph;
│                            Inputs line gains `public/index.html`. NEW, Pass 13: Decision 23's own
│                            "Twelfth-pass broadening" sentence corrected — it had silently redefined
│                            the residual-risk window to start only after Step 4a's decline is
│                            recognized, contradicting the sentence immediately before it (and every
│                            other document's own matching language), which describes the window as
│                            starting while the prompt still awaits input; now stated consistently.
...
│                            (Decision 8/23's own ninth-pass extensions). NEW, Pass 10 (eleventh-pass
│                            correction — this entry was never actually updated for Pass 10's own
│                            real edits to this file until now): header line extended to "ten Design
│                            Remediation Passes"; the Pre-`applying` state-transition bullet gains two
│                            new acknowledged-residual-risk sentences (unbounded confirmation wait;
│                            unhandled SIGINT/SIGTERM during the prompt — Decision 25's own tenth-pass
│                            addenda). NEW, Pass 11: header extended to "eleven Design Remediation
│                            Passes"; the Pre-`applying` bullet's Step 4a decline path now also
│                            appends an `outcome:'cancelled'` audit event, and its own
│                            `scripts/restore.js` citation corrected (`:21-24`). NEW, Pass 12
│                            (thirteenth-pass correction — this entry was never updated for Pass 12's
│                            own real edits to this file, the second of two files this exact
│                            manifest-completeness gap recurred for in that pass): header extended to
│                            "twelve Design Remediation Passes"; the Pre-`applying` bullet's own
│                            "SIGINT/SIGTERM kill" exception language broadened to "any other hard
│                            process termination in the same narrow window," matching the identical
│                            fix in research.md Decision 23 and the restore contract.
...
│                            model" summary with both. NEW, Pass 9 (eleventh-pass correction — this
│                            entry was never actually updated for Pass 9's own real edits to this
│                            file either, a staleness predating and compounding the Pass-10 gap round
│                            11 found): section 5 gains step 9 (backup-file retention, FR-038); new
│                            steps 6d (typed-confirmation gate, FR-039) and 6e (audit-event
│                            requirement's corrected scope/tenant-identity assertion) close this
│                            pass's two CRITICAL findings; step 12a's own assertions extended to
│                            match; a blanket note added near the top requiring
│                            `RESTORE_YES=1`/`--yes` on every step but 6d itself. NEW, Pass 10: header
│                            extended to "Passes 1-10"; section 5 gains step 9a (the hyphen-prefix-
│                            collision regression test for Decision 24's tenth-pass retention fix).
│                            NEW, Pass 11 (twelfth-pass correction — this entry was never actually
│                            updated for Pass 11's own real edits to this file, reproducing the exact
│                            manifest-completeness gap Pass 11 itself claimed to have closed for this
│                            same file's Pass-9/10 content one pass earlier): header extended to
│                            "Passes 1-11"; step 6d's no-response-cancels assertion extended with the
│                            new `outcome:'cancelled'` audit-event check.
├── contracts/
...
│   │                                               12a and Guarantees bullet. NEW, Pass 11
│   │                                               (twelfth-pass correction — Pass 11 itself edited
│   │                                               this contract's own header to backfill the Pass-10
│   │                                               mention omitted above, but this manifest entry was
│   │                                               never updated to say so): header's own pass-list
│   │                                               extended to name "tenth" pass. NEW, Pass 12: the
│   │                                               same header's own pass-list further extended to
│   │                                               "eleventh" (the Pass-11 edit above was itself an
│   │                                               eleventh-pass edit that had not named itself).
...
│                                                    unbounded confirmation-wait composition). NEW,
│                                                    Pass 11: the two Pass-10 residual-risk notes'
│                                                    own `scripts/restore.js` line citation corrected
│                                                    (`:20-23`→`:21-24`, an off-by-one); the audit-
│                                                    event requirement widened again — Step 4a's own
│                                                    decline is a fourth terminal outcome, now audited
│                                                    with its own `outcome:'cancelled'` value, closing
│                                                    a gap where the Pass-9 "exactly one event
│                                                    guaranteed" claim had gone stale the moment Step
│                                                    4a was introduced in that same pass; Test items
│                                                    15/16 extended to assert it; header's stray
│                                                    "(this revision)" tag moved off the Pass 9
│                                                    sentence onto Pass 11's own. NEW, Pass 12: the
│                                                    Pass-11 signal-handling citation's own
│                                                    misattribution corrected (the addendum belongs to
│                                                    Decision 25, not Decision 23 — a mislabel Pass 11
│                                                    itself introduced) and broadened to cover any hard
│                                                    process termination, not signals specifically.
└── tasks.md              # 84 tasks (T001-T075 plus T031a, T034a, T035a, T040a, T053a, T056a,
                             ...
                             NEW, Pass 11 (twelfth-pass correction — this entry was never actually
                             updated for Pass 11's own real edits to this file, the same
                             manifest-completeness gap found elsewhere this pass): T053a/T056d both
                             widened to a fourth audited terminal outcome (Step 4a's own decline,
                             `outcome:'cancelled'`); two Task Quality Gate bullets corrected (a
                             mislabeled Pass-9 origin; a Phase-scoping self-contradiction in the
                             blanket `--yes` bullet). NEW, Pass 12 (own count corrected thirteenth
                             pass — see below): T026's own test-case numbering reconciled with the
                             backup contract's canonical Test contract numbering (was 15 cases
                             numbered 1-14 plus 14a with auditLog exclusion at 13, now 14 cases
                             numbered 1-12 plus 8a/12a, matching the contract exactly); T036 updated
                             to match; every cross-reference to a specific T026 case number updated to
                             the new numbering — T031a (case 10→9), T034a (case 12→11), T035a (cases
                             14/14a→12/12a), and **two** Task Quality Gate bullets (the FR-007
                             implementing-task bullet, case 10→9; the file-mode-fallback bullet, case
                             11→10) — no new task IDs, no test-content change. NEW, Pass 13: the older,
                             historical Pass-8 revision note (this document's own line 19) had one more
                             live cross-reference to T026's pre-renumbering case count ("case 11" for
                             the file-mode-existence-not-enforced test, now case 10) that the Pass-12
                             renumbering sweep missed — closed, matching the Task Quality Gate
                             section's own equivalent, already-corrected bullet.
```
...
│                                          exclusion case (T026 cases 7-8). Pass 7, CRITICAL:
│                                          non-default existence/active-status pre-check case (T026
│                                          case 9). Pass 8: file-mode non-enforcement case (case 10)
│                                          and the audit-event case (case 11). NEW, Pass 9: the
│                                          audit-event case (11) gains an event-tenantId assertion;
│                                          auditLog exclusion case (8a); backup-file retention case
│                                          (12) — 14 cases total (**corrected, Pass 9** — this entry
...
│                                          case (12) extended with a hyphen-prefix-collision case
│                                          (12a — tenants `acme`/`acme-corp`) proving the fix is
│                                          exact-equality, not `startsWith()` — 14 cases total (12
│                                          numbered plus 8a/12a), T036 checkpoint updated to match.
│                                          NEW, Pass 12 (renumbering, not new content): every case
│                                          number above is stated in this document using T026's own
│                                          twelfth-pass-reconciled numbering, matching the backup
│                                          contract's canonical Test contract numbering exactly — this
│                                          entry previously used T026's own now-superseded, higher
│                                          case numbers (case 10 for the existence pre-check, 14/14a
│                                          for retention), which disagreed with the contract by up to
│                                          two positions; see `tasks.md` T026 for the full reconciliation.
...
│                                          NEW, Pass 10, HIGH (eleventh-pass correction — previously
│                                          mislabeled CRITICAL here, contradicting this document's
│                                          own "0 CRITICAL" Pass 10 tally and tasks.md's own matching
│                                          HIGH-labeled Task Quality Gate bullet for the identical
│                                          finding): every pre-existing test in this file other than
│                                          T053a's own
...
│                                          one of them from ever reaching Step 5. NEW, Pass 11: T053a's
│                                          own no-response-cancels test extended to assert exactly one
│                                          `outcome:'cancelled'` audit event with the real target's own
│                                          `tenantId`; T056d's tests extended for this fourth terminal
│                                          outcome (Decision 23's own eleventh-pass widening).
...
## Plan Consistency Review (re-run for Design Remediation Pass 13)
...
- **A "MUST use the canonical audit mechanism on any terminal outcome" requirement's own enumeration of terminal outcomes stays complete as new terminal outcomes are introduced elsewhere in the same pass** (NEW, Pass 11 check, HIGH): Decision 25's Step 4a confirmation gate and Decision 23's "exactly one event is still guaranteed" scope statement were both authored in Pass 9, in different sections of the same document, and were never cross-checked against each other — Step 4a's own ordinary decline path is a fourth terminal outcome the three-outcome enumeration never covered, so a deliberate operator cancellation of a privileged, lock-holding invocation went unaudited by design, not by oversight in implementation. Verified closed via a fourth `outcome:'cancelled'` case, propagated consistently across research.md Decision 23, the restore contract's Mandatory audit event section and Step 4a, T053a/T056d, data-model.md's Pre-`applying` bullet, and spec.md FR-037/FR-039.
- **The exact same self-congratulatory-completeness-claim failure this document set has now caught in Passes 4, 5, 7, 9, and 10 also recurred inside Pass 10 itself** (NEW, Pass 11 check, the meta-finding this round's own dedicated review dimension targeted, again): Pass 10's own commit message claimed "the Documentation manifest... gain[s] Pass 10 entries" for every touched file — but this document's own manifest entries for `data-model.md` and `quickstart.md` were never actually updated, still describing only up-to-Pass-9 content (and, on closer inspection this round, `quickstart.md`'s own manifest entry here had not even been updated for Pass 9's real edits, a staleness one pass older than round 11 initially found). Also recurring: a stray "(this revision)" tag inconsistently left on the superseded Pass-9 paragraph in three of four documents that carry the convention (this document alone had already moved it correctly); an off-by-one source citation copy-pasted verbatim into a second document without being independently checked there; and spec.md's own Pass-9 note carrying the identical severity-count-transposition defect class Pass 10 itself had just fixed, one paragraph above, in the same file. Every instance independently verified and closed this pass — see the Summary section above for the full account.
- **The exact same self-congratulatory-completeness-claim failure recurred inside Pass 11 itself — the fourth consecutive pass to be caught by its own predecessor's version of this same meta-dimension** (NEW, Pass 12 check): Pass 11's own commit message claimed its Documentation manifest fix "corrected above... along with backfilling the identical gap" for `data-model.md`/`quickstart.md` — but this document's own manifest entries for `spec.md`, `quickstart.md` (again — the specific Pass-11 edit was still never entered even after that backfill), `tasks.md`, and `contracts/tenant-backup-contract.md` were never updated for Pass 11's own real edits to all four, despite each genuinely changing in the Pass-11 commit. All four backfilled here. Separately: this document's own row XIII gained a fabricated "CRITICAL-count history" digit sequence in the very sentence written to fix an adjacent overclaim, zeroing out five real CRITICAL findings (Passes 1-5) against this document's own accurate tallies stated a few dozen lines above — corrected. The common root cause remains unchanged from Pass 10's own diagnosis: a claim of completeness substituting for an actual re-check against every document's current state, now observed recurring in four consecutive passes (9, 10, 11, and this round's own catch of Pass 11) — mitigated, not eliminated, by continuing to run a dedicated adversarial dimension against exactly this failure class each round.
- **A citation to "this same decision's own" addendum is verified to actually live in that decision's own section, not merely asserted to** (NEW, Pass 12 check): research.md Decision 23's new Step 4a residual-risk sentence cited the SIGINT/SIGTERM addendum as "this same decision's own tenth-pass residual-risk addendum, above" — but that addendum lives entirely under Decision 25, a separate section two decisions later, not anywhere inside Decision 23 itself. Verified closed by correcting the citation to point to Decision 25, propagated everywhere the same misattributed framing (and its overly narrow "signal kill" scoping) had been copied: the restore contract, data-model.md, tasks.md, and spec.md.
- **Two documents both claiming to be the authoritative numbering for one test suite actually agree, not merely both claim to** (NEW, Pass 12 check, HIGH; own citation corrected Pass 13): `tasks.md`'s T026/T036 numbered `tests/tenant-backup.test.js`'s cases 1-through-14-plus-14a (15 items total, `auditLog` exclusion at case 13), while `contracts/tenant-backup-contract.md`'s own Test contract numbered the identical suite 1-through-12-plus-8a/12a (14 items total, `auditLog` exclusion at 8a). Verified this was a genuine, silent divergence (not merely a citation typo, since the two documents disagreed on total count and item position, not just a label), reconciled by renumbering `tasks.md` to match the contract's own numbering exactly — no test content changed. (**Thirteenth-pass correction**: this bullet's own draft, written in the same pass as the fix, still cited research.md's header as saying "(12a/14a)" — the pre-fix dual number — instead of noting that this very fix made both documents agree on 12a; corrected.)
- **A "broadening" correction to an exception's own scope does not, in the same breath, silently narrow the window it applies to** (NEW, Pass 13 check, HIGH): research.md Decision 23's own "Twelfth-pass broadening" sentence, written to generalize an exception from "signal kill" to "any hard process termination," redefined the window in the same sentence — from "while the prompt awaits input" (the sentence immediately before it, and every one of the four documents this fix was propagated to) to "after Step 4a's decline is recognized" (materially later, excluding the primary scenario). Verified this was a genuine self-contradiction within the same paragraph, not a stylistic variance, and corrected to state the same window as the surrounding text and all four propagated copies.
- **A pass's own commit-message claim to have backfilled every stale manifest entry is checked against every file that pass actually touched, not only the files the claim names** (NEW, Pass 13 check, the meta-finding this round's own dedicated review dimension targeted, a fifth consecutive recurrence — Passes 9, 10, 11, and 12 each caught an instance of this identical failure class, now a sixth instance in Pass 13's own catch of Pass 12): Pass 12's commit message named four files (`spec.md`, `quickstart.md`, `tasks.md`, `contracts/tenant-backup-contract.md`) as backfilled — and all four manifest entries were indeed genuinely updated — but `research.md` and `data-model.md`, the two files Pass 12 edited most substantively (the Decision 23 citation fix and new residual-risk paragraph; the matching Pre-`applying` broadening), were never checked against the manifest at all, this pass's own dedicated review dimension having been scoped to "the files the commit message names" rather than "every file `git show` confirms was touched." Both backfilled here; the spec.md and tasks.md entries — which the commit message DID name, but only partially — are also expanded to credit every real edit, not only the first one found.
- **A syntactically-balanced sentence stays balanced after a mid-sentence insertion** (NEW, Pass 13 check, HIGH): spec.md's FR-037 had an outer parenthetical — `(including a Step 1 validation-gate rejection...)` — that Pass 12's own text insertion left permanently unclosed, because the new `(**pass 12**: ...)` aside consumed the position of what had been the outer group's own closing paren without also supplying one. Verified by independently re-deriving the paren nesting depth of the full requirement text and confirming it did not return to zero before the sentence's next major clause; corrected by adding the outer group's own closing paren back, immediately after the new aside's own closing paren.
