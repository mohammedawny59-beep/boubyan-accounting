...
| XIII. Spec Kit Usage | **PASS** | `/speckit-analyze` → Pass 1 → `/speckit-analyze` → Pass 2 → `/speckit-analyze` → Pass 3 → `/speckit-analyze` → Pass 4 → `/speckit-analyze` → Pass 5 → `/speckit-analyze` → Remediation Pass 6 (this document's current state) → a seventh `/speckit-analyze` re-verification next, per the user's own gate — the process continues to work exactly as intended: each re-verification round, including of Pass 3's own citation "fixes" and Pass 4's own incomplete cache-staleness fix, has caught what the round before it missed, converging (17 → 13 → 9 → 11 → 9 → 8 gate-blocking findings) toward zero — Pass 6 is also the first round to find 0 CRITICAL findings. |
...
scripts/tenant-restore.js          # NEW (E/F/G/H). Step -1 (NEW, Pass 5, CRITICAL): a loud,
                                    ...
                                    # back (Pass 5, HIGH; rationale corrected Pass 6 — see T056c).
...
docs/PRODUCTION_RUNBOOK.md         # NEW section (research.md Decision 18): documents the always-
                                    ...
                                    # naming both sourceId and journalId (Decision 20, FR-034). NEW,
                                    # Pass 5: a second mandatory numbered step, placed before the
                                    # restart step in the operator's run order, for the pre-restore
                                    # quiesce requirement (Decision 19's fifth-pass extension); a
                                    # cross-reference to the tool's own restore-time idempotency count
                                    # (T056b); a note that the local staging file is deleted
                                    # automatically on success; and a correction to this runbook's own
                                    # pre-existing, feature-unrelated §5.1 claim that no live scheduled
                                    # job exists in this repository — false, given startBackupSchedule()
                                    # at server.js:9942 — plus an operational note about that scheduler
                                    # potentially running concurrently with a tenant-scoped restore.
...
## Plan Consistency Review (re-run for Design Remediation Pass 6)
...
- **A tool's own design manifest (this document) describes what its own tasks actually require** (NEW, Pass 6 check, HIGH): the Source Code section's `scripts/tenant-restore.js` and `docs/PRODUCTION_RUNBOOK.md` entries were found stale at Pass 4 content despite both files' tasks being substantially extended in Pass 5 — corrected above; a reviewer relying on this document's own manifest, rather than reading tasks.md/research.md in full, would otherwise have missed three Pass-5 requirements specific to those two files.
- **An operator-facing count queries the representation it can actually trust** (NEW, Pass 6 check): the idempotency-staleness count (Decision 20's fifth-pass extension) is verified to name the dedicated `IdempotencyRecord` collection specifically, not the `EntityChunk`-embedded array that can briefly disagree with it — consistent across research.md Decision 20, the restore contract's Step 6, data-model.md, and `tasks.md` T056b.
- **A decision's own claim about execution order stays true as later decisions change that order** (NEW, Pass 6 check): Decision 11's lock-acquisition-order language and Decision 8's "never queried... at all" claim were both found stale after Decision 19 (Step -1) and Decision 20 (the Step 6 count) were introduced ahead of and after them respectively — both corrected directly in research.md, with no downstream document found to repeat either stale claim.
- **The mandatory crash-window test can actually land its own target window, not just describe it** (NEW, Pass 6 check, MEDIUM): T058 is verified to now specify the same class of documented test-only hook its sibling T057 already uses, removing a wall-clock-timing flakiness gap the test previously had no mechanism to avoid.
