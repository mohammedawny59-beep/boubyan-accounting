...
**Revised through Design Remediation Passes 1-12** — steps 6-8 rewritten for the offline-staging redesign; steps 9-12 added for the restore lock, digest-based resume, backup fingerprint, and default-duplicate pre-flight; Pass 2 adds the `--force-unlock`-after-any-crash requirement to steps 7-8, corrects the checkpoint/staging paths to be tenant-keyed, adds an `entityChunks` key-scoping check to step 6, and adds a "Recovery model" summary; **Pass 3 made no change to this file** (its fixes were internal to the digest-computation helper and did not change any operator-facing step); Pass 4 adds step 6a (verify the restart-requirement message) and step 6b (verify the idempotency-staleness note is documented), and extends the "Recovery model" summary below; Pass 5 adds step 5a (verify the new pre-restore quiesce warning), corrects step 6a's own verification wording (it previously asked to confirm the opposite of the actual risk), and adds step 6c (verify the idempotency-staleness count and staging-file cleanup); Pass 6 corrects step 6c's own rationale for why the staging file survives a failed run (it is not read back on resume), and extends step 6b to also check the runbook names both `sourceId` and `journalId`; Pass 7 adds section 5's step 7 (the non-default existence/active-status pre-check, FR-007/FR-035), and notes throughout that every operator-facing message verified by these steps is now Arabic, matching `scripts/restore.js`'s own convention (Decision 22, FR-036) — Passes 1-6 left every illustrative example in English; Pass 8 adds section 5's step 8 (the withdrawn file-mode existence heuristic) and step 12a (the audit-event requirement, FR-037), and corrects two English placeholders in step 6c that Pass 7's own Arabic sweep missed; Pass 9 adds section 5's step 9 (backup-file retention, FR-038), a new step 6d (the typed-confirmation gate, FR-039) and step 6e (the audit-event requirement's corrected scope and tenant-identity assertion) — closing this pass's two CRITICAL findings — and extends step 12a's own assertions to match; Pass 10 adds section 5's step 9a (the hyphen-prefix-collision regression test, closing a real HIGH-severity bug in Pass 9's own new retention mechanism, research.md Decision 24's tenth-pass correction) — no other quickstart.md step required correction, since the blanket `RESTORE_YES=1` note added in Pass 9 (below) already covered every step's own immunity to Pass 9's new confirmation gate; Pass 11 extends step 6d's own no-response-cancels assertion with the new `outcome:'cancelled'` audit-event check (research.md Decision 23's own eleventh-pass widening, closing a gap where an operator's ordinary decline at Step 4a had gone unaudited since the moment that gate was introduced) — no other step required correction; **Pass 12 made no change to this file (thirteenth-pass note, closing a gap where this header line's own established convention — explicitly noting a no-change pass, as it did for Pass 3 — was not followed for Pass 12): Pass 12's own fixes were citation/scope corrections to research.md, the restore contract, data-model.md, tasks.md, and spec.md that do not touch any operator-facing quickstart.md step.**

All steps run against an **isolated local/test environment** — `DB_FILE_ONLY=true` or a `mongodb-memory-server` instance via `tests/helpers/mongoTestHarness.js`. None touch production or demo. **NEW, Pass 9**: every `tenant-restore.js` invocation in the steps below other than step 6d itself assumes `RESTORE_YES=1` (or `--yes`) is set, per the new typed-confirmation gate (Decision 25, FR-039) — otherwise every one of them would pause for interactive input.
...
9. **Backup-file retention (NEW, Pass 9, MANDATORY — FR-038)**: with `TENANT_BACKUP_KEEP=3` set, run the backup command 5 times for `acme`; confirm exactly 3 `tenant-acme-*.json` files (+ `.sha256` sidecars) remain afterward, the 3 newest by timestamp; confirm a *different* tenant's own backup files, and any whole-instance `backup-*.json` file, are completely unaffected.
9a. **Backup-file retention, hyphen-prefix collision (NEW, Pass 10, MANDATORY — closes a real HIGH-severity design bug in Pass 9's own retention mechanism, research.md Decision 24's tenth-pass correction)**: with two tenants `acme` and `acme-corp` (one a literal string-prefix of the other) and `TENANT_BACKUP_KEEP=3` set, run the backup command 5 times for `acme-corp`, then once for `acme`; confirm `acme-corp` retains exactly 3 files and `acme` retains exactly 1 — a naive `startsWith('tenant-acme-')` match would have let `acme`'s rotation delete `acme-corp`'s own backups, since `tenant-acme-corp-<stamp>.json` literally starts with `tenant-acme-`.
10. **`auditLog` exclusion (NEW, Pass 9 — research.md Decision 8's ninth-pass extension)**: seed an `EntityChunk` keyed `auditLog` for `acme` (real audit history); back it up; confirm it does not appear anywhere in the output, mirroring step 6's own `idempotencyRecords` case.
...
## 6d. Verify the typed-confirmation gate (NEW, Pass 9, CRITICAL — research.md Decision 25, FR-039; audit-event assertion added Pass 11)

1. Start `tenant-restore.js` for `acme` with no `--yes` and no `RESTORE_YES` set; confirm the process pauses after the checkpoint is written and before any write to `acme`'s data, echoing `acme` and the `--target=` label; type anything other than `نعم` (or send EOF); confirm the process exits `0`, the lock is released, the checkpoint is left at `stage:'staged'`, zero writes occurred to `acme`'s data, and — **NEW, Pass 11 (research.md Decision 23's own eleventh-pass widening)** — exactly one `outcome:'cancelled'` event was appended to `default`'s own `auditLog`, with the event's own `tenantId` field equal to `acme` (not `'default'`).
2. Repeat, this time typing `نعم`; confirm Step 5 proceeds normally to completion.
3. Repeat with `--yes` passed, and separately with `RESTORE_YES=1` set; confirm both skip the prompt entirely and proceed non-interactively — confirming the mandatory automated tests elsewhere in this suite (steps 7-9 below) are not themselves blocked by this new gate.

## 6e. Verify the audit-event requirement's corrected scope (Pass 8; scope and mechanics corrected Pass 9 — research.md Decision 23, FR-037)

1. Run a successful `tenant-restore.js` for `acme`; confirm `default`'s own `auditLog` gained exactly one new entry with `action:'tenant.restore'`, `resourceId:'acme'`, `outcome:'success'`, **and the event's own `tenantId` field equal to `'acme'`, not `'default'`** (Pass 9 — confirming the synthetic-`req` construction is actually wired in); confirm `acme`'s own `auditLog` gained no such entry (seed `acme` with a pre-existing entry beforehand, so its absence there is a real assertion).
2. **NEW, Pass 9**: force a Step 1 validation-gate rejection (e.g. a tampered backup file) and, separately, a Step 2 default-duplicate rejection (target `default`); confirm each independently appends exactly one `outcome:'failure'` event to `default`'s own `auditLog` — closing the gap where only Step 5/6 outcomes were previously audited.
3. **NEW, Pass 9**: run a restore targeting `default` itself; confirm `default`'s own `auditLog` — including every prior event, this restore's own included — survives byte-for-byte except for the one new event this restore itself appends, proving the `TENANT_BACKUP_ENTITY_KEYS` `auditLog` exclusion actually protects the centralized log from the exact self-destruction scenario Decision 23 exists to prevent.
...
2. Confirm exactly one acquires the lock and proceeds; the other is rejected immediately (before it even opens the backup file), naming the held lock's `runId`/`pid`/age and the exact `--force-unlock --expected-lock-run-id=<runId>` command required for an intentional takeover.
3. After the first completes (lock released), confirm a third invocation now succeeds normally.
4. Kill a run mid-apply (a real process kill). Confirm a plain re-run (no `--force-unlock`) is rejected at Step 0, naming the dead run's `runId`/`pid`/age. Confirm a bare `--force-unlock` (no `--expected-lock-run-id=`) is also rejected, before Step 0, touching nothing. Confirm a re-run **with `--force-unlock --expected-lock-run-id=<the exact runId just reported>`** succeeds.
5. Confirm a force-takeover naming the WRONG expected `runId` (not the one currently held) fails without mutating the real lock, and confirm two processes force-takeover-ing the SAME expected stale `runId` near-simultaneously resolve to exactly one winner, purely via MongoDB's own atomicity — no timing/delay required for this to hold reliably.
...
3. Re-run **with `--force-unlock --expected-lock-run-id=<the crashed run's own runId, from a plain re-run's rejection message>`** (required after any real process kill — there is no exception for this scenario, per the corrected lock policy; a bare `--force-unlock` is rejected before Step 0) with the **same** backup file. Confirm: `users` is **not** redundantly deleted/reinserted (its live digest already matches `expected`), while `entityChunks`/`appConfigs` proceed normally to completion.
...
## 12a. Verify the audit-event requirement, backup side (NEW, Pass 8 — FR-037, per this project's own constitution Principle XI)

1. Run a successful `tenant-backup.js` for `acme`; confirm `default`'s own `auditLog` gained exactly one new entry with `action:'tenant.backup'`, `resourceId:'acme'`, `outcome:'success'`, and the event's own `tenantId` field equal to `'acme'`, not `'default'` (Pass 9) — and confirm `acme`'s own `auditLog` gained **no** such entry.
2. Force a hard-failed backup (e.g. the existence/active-status pre-check, step 7 above); confirm `default`'s `auditLog` gained exactly one entry with `outcome:'failure'` instead.
3. **For the restore-side equivalent, including the Pass-9 scope correction (Step 1/Step 2 rejections, not only Step 5/6 outcomes) and the `default`-restore self-destruction check, see step 6e above.**

## Recovery model (stated once, plainly, for operators)

**Resume is supported. Rollback is not.** After any restore that exits cleanly (a caught failure), a plain re-run with the same backup file resumes correctly, skipping whatever the digest re-check confirms is already applied. After any restore that is killed rather than exiting cleanly, resuming requires `--force-unlock --expected-lock-run-id=<RUN_ID>` — the exact identity of the crashed run's own lock, normally read from a plain re-run's own rejection message — with no exception, even for an otherwise-routine crash. A bare `--force-unlock` naming no expected identity is rejected before Step 0: the tool never infers which lock to replace by rereading whatever currently exists, because the operator's own explicit identification of the lock — not the tool's guess about its age — is what makes an intentional takeover safe (owner-mandated deterministic-lock redesign, post-lean-review; mutual exclusion must never depend on process timing). There is no automatic way to revert a category that already applied successfully back to its pre-restore state; reverting means restoring again, deliberately, from a backup taken before the unwanted one.
